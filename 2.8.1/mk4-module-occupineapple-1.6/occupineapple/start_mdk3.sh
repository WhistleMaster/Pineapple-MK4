...
MYMONITOR=`cat ${MYPATH}occupineapple.conf | grep "monitor" | awk -F = '{print $2}'`
MYINTERFACE=`cat ${MYPATH}occupineapple.conf | grep "interface" | awk -F = '{print $2}'`

MYLIST=`cat ${MYPATH}occupineapple.conf | grep "list" | awk -F = '{print $2}'`
SPEED=`cat ${MYPATH}occupineapple.conf | grep "speed" | awk -F = '{print $2}'`
CHANNEL=`cat ${MYPATH}occupineapple.conf | grep "channel" | awk -F = '{print $2}'`
MYOPTIONS=`cat ${MYPATH}occupineapple.conf | grep "options" | awk -F = '{print $2}'`
