...
TO=`cat ${MYPATH}infusion.conf | grep "to" | awk -F = '{print $2}'`
FROM=`cat ${MYPATH}infusion.conf | grep "from" | awk -F = '{print $2}'`
SUBJECT=`cat ${MYPATH}infusion.conf | grep "subject" | awk -F = '{print $2}'`
