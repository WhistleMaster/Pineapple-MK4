	        ...
	        <div class="container">
	          ...
	          <div class="nav-collapse collapse">
				  <ul class="nav">
					  ...
					  <li class="active"><a href="./ssid.php"><i class="icon-signal"></i>&nbsp;SSID</a></li>
	    ...
	    <div class="container well">
			...
			<div class="row">
				<div class="span12">
					...
					<li class="active"><a href="#ssid">SSID Repartition</a></li>
					<li><a href="#signal">Average Signal Quality</a></li>
					</ul>
					<div class="tab-content">
					<div class="tab-pane active" id="ssid"><div id="chart_ssid" style="width:100%; height:400px;"></div></div>
					<div class="tab-pane" id="signal"><div id="chart_signal" style="width:1170px; height:400px;"></div></div>
					</div>
				</div>
			</div>
			...
			<?php
				...
				if(login_check($mysqli) == true)
				{
					...
					$result_signal = $mysqli->query("
						SELECT 
							DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') AS Signal_Date, 
							Station_SSID, 
							AVG(Station_Signal_Quality) AS Signal_Average
						FROM 
							PineappleStats_Data_View
						WHERE 
							DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') >= '".$start_date."' AND
							DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') <= '".$end_date."'
							".$pineapple_filter."
						GROUP BY Signal_Date, Station_SSID
					");
					...
					<table cellpadding="0" cellspacing="0" border="0" class="display" id="grid" width="100%">
						...
						<?php 
							...
							while($row = $result->fetch_assoc())
							{
								$ssid[] = $row['Station_SSID'];
							}
							
							while($row = $result_signal->fetch_assoc())
							{
								$signal[$row['Station_SSID']][strtotime($row['Signal_Date'])*1000] = $row['Signal_Average'];
							}
						?>
						</tbody>
						<tfoot>
		...
		var oTable;
			...
			$(function () { 
			    $('#chart_ssid').highcharts({
		            ...
		            plotOptions: {
						...
						series: {
			                ...
			                point: {
			                    events: {
			                        click: function() {
										oTable.fnReloadAjax('getdata.php?Station_SSID='+this.name);
			                        }
			                    }
			                }
			            }
					...
					series: [{
		                type: 'pie',
		                name: 'SSID Distribution',
		                data: [
						<?php
						
							$i=1; $all=count(array_count_values($ssid));
							foreach(array_count_values($ssid) as $key => $value)
							{
								echo "['".$key."', ".$value."]";
								if($i != $all) echo ",";
			...
			$(function () {
		        ...
		        $('#chart_signal').highcharts({
		            chart: {
		                type: 'column',
						zoomType: 'x'
		            },
		            title: {
		                text: 'Average Signal Quality'
		            },
					xAxis: {
				        type: 'datetime',
