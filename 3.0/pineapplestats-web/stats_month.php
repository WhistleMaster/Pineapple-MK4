	...
	<div class="container well">
		...
		<div class="row">
			...
			<?php
				...
				if(login_check($mysqli) == true)
				{
					...
					$result = $mysqli->query("
						SELECT
					 		DATE_FORMAT(`Data_Timestamp`,'%Y-%m') AS `Unique_Date`,
					...
					$result = $mysqli->query("
						SELECT 
							DATE_FORMAT(`Data_Timestamp`,'%Y-%m') AS `Visit_Date`,
					...
					$result = $mysqli->query("
						...
						FROM ( 
							SELECT 
								DATE_FORMAT(`Data_Timestamp`,'%Y-%m') as Average_Visit_Date, 
					...
					$result = $mysqli->query("
						...
						(
							SELECT
								...
								) AS min ON min.`Station_MAC` = PineappleStats_Data_View.`Station_MAC` and min.Visit_Date = DATE_FORMAT(PineappleStats_Data_View.`Data_Timestamp`,'%Y-%m')
								WHERE
									DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') >= '".$start_date."' AND
									DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') <= '".$end_date."'
									".$pineapple_filter."
					...
					$result = $mysqli->query("
						...
						(
							SELECT
								DATE_FORMAT(`Stats_Timestamp`,'%Y-%m') AS `Visit_Date`,
		...
		$(function () {
	        ...
	        $('#chart_unique').highcharts({
	            ...
	            tooltip: {
	                formatter: function() {
	                    return '<b>'+ Highcharts.dateFormat('%Y-%m', this.x) +'</b><br/>'+
