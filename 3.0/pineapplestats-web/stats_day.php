...
<body>
	<div class="navbar navbar navbar-fixed-top">
		...
		<div class="container well">
			...
			<div class="row">
				<div class="span12">
					...
					<div class="tab-pane active" id="unique_t">
						...
						<div id="chart_unique_hour" style="width:1170px; height:400px;"></div><br/>
			...
			<?php
				...
				if(login_check($mysqli) == true)
				{
					...
					$result = $mysqli->query("
						SELECT
					 		DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') AS `Unique_Date`,
					...
					//Unique Stations per Hour
					$result = $mysqli->query("
						SELECT   
							CONCAT(HOUR(`Data_Timestamp`), ':00-', HOUR(`Data_Timestamp`)+1, ':00') AS Hours,
							COUNT(DISTINCT `Station_MAC`) AS `Unique_Number` 
						FROM PineappleStats_Data
						WHERE 
							DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') >= '".date("Y-m-d")."' AND
							DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') <= '".date("Y-m-d")."'
							".$pineapple_filter."
						GROUP BY HOUR(`Data_Timestamp`)
					");
					
					$uniques_per_hour=array();
					
					while($row = $result->fetch_assoc())
					{
						$uniques_per_hour[$row['Hours']] = $row['Unique_Number'];
					}

					// Visit
					$result = $mysqli->query("
						SELECT 
							DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') AS `Visit_Date`,
					...
					$result = $mysqli->query("
						...
						FROM ( 
							SELECT 
								DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') as Average_Visit_Date, 
					...
					$result = $mysqli->query("
						...
						(
							SELECT
								...
								) AS min ON min.`Station_MAC` = PineappleStats_Data_View.`Station_MAC` and min.Visit_Date = DATE_FORMAT(PineappleStats_Data_View.`Data_Timestamp`,'%Y-%m-%d')
								WHERE 
									DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') >= '".$start_date."' AND
									DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') <= '".$end_date."'
									".$pineapple_filter."
					...
					$result = $mysqli->query("
						...
						(
							SELECT
								DATE_FORMAT(`Stats_Timestamp`,'%Y-%m-%d') AS `Visit_Date`,
		...
		$(function () {
	        ...
	        $('#chart_unique_hour').highcharts({
	            chart: {
	                type: 'column'
	            },
	            title: {
	                text: 'Number of unique Stations per Hour (today)'
	            },
				xAxis: {
			        type: 'category'
			    },
	            ...
	            series: [{
					...
					<?php
						...
						$i=1; $all=count($uniques_per_hour);
						foreach($uniques_per_hour as $key => $value)
						{
							echo "['".$key."', ".$value."]";
							if($i != $all) echo ",";
