		...
		<div class="row">
			...
			<?php
				...
				if(login_check($mysqli) == true)
				{
					...
					$result = $mysqli->query("
						SELECT
					 		DATE_FORMAT(`Data_Timestamp`,'%Y-%u') AS `Unique_Date`,
					...
					while($row = $result->fetch_assoc())
					{
						$uniques[$row['Unique_Date']] = $row['Unique_Number'];
					}
					
					// Visit
					$result = $mysqli->query("
						SELECT 
							DATE_FORMAT(`Data_Timestamp`,'%Y-%u') AS `Visit_Date`,
					...
					while($row = $result->fetch_assoc())
					{
						$visits[$row['Visit_Date']] = $row['Visit_Number'];
					}
					...
					while($row = $result->fetch_assoc())
					{
						$visits_pineapple[$row['Pineapple_Name']][$row['Visit_Date']] = $row['Visit_Number'];
					}
					...
					$result = $mysqli->query("
						...
						FROM ( 
							SELECT 
								DATE_FORMAT(`Data_Timestamp`,'%Y-%u') as Average_Visit_Date, 
					...
					while($row = $result->fetch_assoc())
					{
						$average_visits[$row['Average_Visit_Date']] = $row['Average_Visit_Duration'];
					}
					...
					$result = $mysqli->query("
						...
						(
							SELECT
								...
								) AS min ON min.`Station_MAC` = PineappleStats_Data_View.`Station_MAC` and min.Visit_Date = DATE_FORMAT(PineappleStats_Data_View.`Data_Timestamp`,'%Y-%u')
								WHERE
									DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') >= '".$start_date."' AND
									DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') <= '".$end_date."'
									".$pineapple_filter."
					...
					$result = $mysqli->query("
						...
						(
							SELECT
								DATE_FORMAT(`Stats_Timestamp`,'%Y-%u') AS `Visit_Date`,
