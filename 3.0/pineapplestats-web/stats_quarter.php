		...
		<div class="row">
			...
			<?php
				...
				if(login_check($mysqli) == true)
				{
					...
					$result = $mysqli->query("
						SELECT
					 		QUARTER(`Data_Timestamp`) AS `Unique_Date`,
					...
					$result = $mysqli->query("
						SELECT 
							QUARTER(`Data_Timestamp`) AS `Visit_Date`,
					...
					$result = $mysqli->query("
						...
						FROM ( 
							SELECT 
								QUARTER(`Data_Timestamp`) AS Average_Visit_Date, 
					...
					$result = $mysqli->query("
						...
						(
							SELECT
								...
								) AS min ON min.`Station_MAC` = PineappleStats_Data_View.`Station_MAC` and min.Visit_Date = QUARTER(PineappleStats_Data_View.`Data_Timestamp`)
								WHERE
									DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') >= '".$start_date."' AND
									DATE_FORMAT(`Data_Timestamp`,'%Y-%m-%d') <= '".$end_date."'
									".$pineapple_filter."
					...
					$result = $mysqli->query("
						...
						(
							SELECT
								QUARTER(`Stats_Timestamp`) as Visit_Date,
	            ...
	            series: [
					...
					<?php
						...
						foreach($visits_pineapple as $key => $value)
						{
							...
							{
								echo "['Q".$key_timestamp."', ".$value_signal."]";
								if($j != $all_data) echo ",\n";
