...
if(isset($_GET['interface']))
{
	...
	for($i=0;$i<$nbr_wifi_devices;$i++)
	{
		...
		echo '<td colspan="2">';
		if($interface != "-")
			echo ' ['.$mac_address.'] <a id="wifimanager_macchanger" href="javascript:wifimanager_macchanger(\''.$interface.'\',\''.$i.'\');">[Random Mac]</a>'; 
		else
			echo '';
		echo '</td>';
    ...
    for ($i=0;$i<count($wifi_interfaces);$i++)
    {
		...
		{
			if($ip_address != "")
				echo '<td><a id="wifimanager_release" href="javascript:wifimanager_release(\''.$wifi_interfaces[$i].'\');">[DHCP Release]</a></td>';
			else
				echo '<td><a id="wifimanager_connect" href="javascript:wifimanager_connect(\''.$wifi_interfaces[$i].'\');">[DHCP Request]</a></td>';
		}
		else
		{
			echo '<td>&nbsp;</td>';
