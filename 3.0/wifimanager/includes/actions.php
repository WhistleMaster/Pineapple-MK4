...
if (isset($_POST['conf']))
{
	...
	if($_POST['conf'] == "ics")
	{
		...
		$newdata = "from=".$interface_from."\n"."to=".$interface_to."\n"."first_backup=".$first_backup_done;
		...
		if($interface_from != "none" && $interface_to != "none")
		{
			...
			exec("sed -i 's/FROMINTERFACE=\(.*\)/FROMINTERFACE=".$interface_from."/g' ".$directory."includes/autostart.sh");
			exec("sed -i 's/TOINTERFACE=\(.*\)/TOINTERFACE=".$interface_to."/g' ".$directory."includes/autostart.sh");
			...
			{
				exec("sed -i '/wifimanager\/includes\/autostart.sh/d' /etc/rc.local");
			}
		}
		else
		{
			exec("iptables -F");
			exec("sed -i '/wifimanager\/autostart.sh/d' /etc/rc.local");
		}
